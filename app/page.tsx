...
function Brand(){return <a href="#top" className="brand" aria-label="Syntra Optimizer home"><img src="/assets/syntra-logo.png" width="32" height="32" alt=""/><span>Syntra<span className="brand-sub"> Optimizer</span></span></a>}
...
export default function Home(){
 ...
 return <><a className="skip-link" href="#main">Skip to content</a><div id="top" className="site-shell"><header className={scrolled?'navigation scrolled':'navigation'}><Brand/><nav aria-label="Main navigation" className={menu?'nav-links open':'nav-links'}>{[['Features','features'],['Safety','safety'],['For you','profiles'],['Pricing','pricing']].map(([label,id])=><a key={id} href={`#${id}`} onClick={()=>setMenu(false)}>{label}{['Features','For you'].includes(label)&&<ChevronDown size={12}/>}</a>)}</nav><div className="nav-actions">{user?<button onClick={()=>setAccount(true)} className="nav-user-chip" aria-label="Open account details"><span className="nav-user-avatar">{userAvatar?<img src={userAvatar} alt="" referrerPolicy="no-referrer"/>:userInitial}</span><span className="nav-user-name">{userName}</span><span className={`nav-plan-pill ${plan==='Premium'?'premium':'free'}`}>{plan==='Premium'?'✦ Premium':'Free'}</span></button>:<button onClick={()=>setAccount(true)} className="nav-demo">Sign in</button>}<button className="button primary compact" onClick={()=>{if(product.downloadUrl)window.location.assign(product.downloadUrl);else setDownload(true)}}>Get Syntra<ArrowUpRight size={14}/></button><button className="menu-toggle" onClick={()=>setMenu(!menu)} aria-label="Toggle navigation" aria-expanded={menu}>{menu?<X/>:<Menu/>}</button></div></header><GradualBlur position="bottom" target="page" height="5rem" strength={2.5} divCount={6} curve="ease-out" exponential opacity={1} zIndex={-70}/><main id="main"><section className="hero"><div className="hero-veil" aria-hidden="true"><DarkVeil hueShift={30} noiseIntensity={0.025} scanlineIntensity={0} speed={0.3} warpAmount={0.15} resolutionScale={0.75} lightMode={light}/></div><div className="hero-copy"><a href="#features" className="announcement">Meet your PC’s new potential <ChevronRight size={13}/></a><h1>Less friction. More performance.<br/><span>Your PC, unleashed.</span></h1><p>A cleaner system. A smoother experience. Take control of your<br className="desktop-break"/> Windows PC with Syntra Optimizer.</p><div className="hero-actions"><button className="button primary" onClick={()=>{if(product.downloadUrl)window.location.assign(product.downloadUrl);else setDownload(true)}}>Get Syntra Optimizer <ArrowRight size={17}/></button><a className="button secondary" href="#demo"><Play size={14}/>See how it works</a></div><span className="hero-note"><Monitor size={13}/> Built for Windows <span>·</span> Your settings. Your choice.</span></div><AppPreview/></section><Sections onDownload={()=>setDownload(true)} onAccount={()=>setAccount(true)}/></main><footer><Brand/><span>© {new Date().getFullYear()} Syntra Optimizer</span><a href="#top">Back to top ↑</a></footer></div><button className="theme-toggle" onClick={()=>setLight(!light)} aria-label={light?'Switch to dark theme':'Switch to light theme'}>{light?<Moon size={19}/>:<Sun size={19}/>}</button><Account open={account} onOpenChange={setAccount} onUpgrade={()=>{const el=document.getElementById('pricing');if(el)el.scrollIntoView({behavior:'smooth'})}}/><Dialog open={download} onOpenChange={setDownload}><DialogContent className="download-dialog"><img src="/assets/syntra-logo.png" width="56" height="56" alt="Syntra logo"/><DialogTitle className="dialog-title">Your next-level PC is on its way.</DialogTitle><DialogDescription>The Syntra Optimizer download is coming soon. In the meantime, explore the interactive preview and see what a more focused PC experience can look like.</DialogDescription><a className="button primary" href="#demo" onClick={()=>setDownload(false)}>Explore the demo<ArrowRight size={16}/></a><span className="muted">No installer is available on this website yet.</span></DialogContent></Dialog></>
}
